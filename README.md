...
Jenkins and Slaves are running on EC2 free tier instances, which are configured to use really low resources (memory and disk). Processing many jobs at a time might take time. and sometimes when retriving docker images, slaves may run out of space causing the build to fail.
